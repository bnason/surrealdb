...
fn parse_select_with_order_by_params() {
	let res = test_parse!(parse_stmt, r#"SELECT * FROM users ORDER BY $order_by $order_direction"#).unwrap();
	...
	if let Some(Ordering::Order(OrderList(orders))) = stmt.order {
		...
		assert_eq!(
			order.value,
			Value::Idiom(Idiom(vec![Part::Start(Value::Param(Param(Ident("order_by".to_string()))))]))
		);
		
		// Check direction is a parameter
		assert_eq!(
			order.direction,
			Value::Param(Param(Ident("order_direction".to_string())))
		);
	} else {
